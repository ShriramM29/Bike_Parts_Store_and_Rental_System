...
<table width="80%" bgcolor="#FFFFFF">
  ...
  <tr>
    <td colspan="2" style="font-size:28px;color:#3300CC; font-weight:bold" align="center"> Spare Delivery Entry </td>
    </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  ...
  <tr>
	...
	<select name="select" id="select" onchange="MM_jumpMenu('parent',this,0)">
                  ...
                  <?php
						include("Home.php");
						$q5=mysql_query("select ID from SpareOrder where Status='Pending'");
						while($r=mysql_fetch_assoc($q5))
						{
							echo "<option value=Delivery.php?bid=".$r["ID"].">".$r["ID"]."</option>";
						}
						$id=$_REQUEST['bid'];
   ...
   <?php $result = mysql_query("SELECT * FROM  SpareOrder where ID='$id' ");
  ...
  <tr>
    <td>Spare ID </td>
    <td><input type="text" name="bid" value="<?php echo $row1["SpareID"]; ?>" /></td>
  </tr>
  <tr>
  
    <td>Name</td>
    <td><input type="text" name="title" value="<?php echo $row1["Name"]; ?>" /></td>
  </tr>
  <tr>
    <td>Company</td>
    <td><input type="text" name="author"  value="<?php echo $row1["Company"]; ?>"  /></td>
  </tr>
  <tr>
    <td>Model</td>
    <td><input type="text" name="p_year"  value="<?php echo $row1["Model"]; ?>"/></td>
  </tr>
  <tr>
    <td>Price </td>
...
if(isset($_POST['Submit']))
{
	...
	$result=mysql_query("update SpareOrder set Status='Delivered' where ID='$oid'");
